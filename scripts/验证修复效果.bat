...
set WALLET_NAME=verify_wallet_%RANDOM%
curl -s -X POST "%API_BASE%/api/wallets" -H "Authorization: Bearer %API_KEY%" -H "Content-Type: application/json" -d "{\"name\": \"%WALLET_NAME%\", \"description\": \"验证钱包\", \"quantum_safe\": false}" | findstr /C:"address" >nul
if %errorlevel% equ 0 (
    echo ✅ 创建钱包 - 成功
    set /a SUCCESS_COUNT+=1
) else (
