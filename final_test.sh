...
run_test() {
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] 测试: $test_name"
...
# 测试3: 创建钱包
test_create_wallet() {
    local wallet_name="final_test_wallet_$(date +%s)"
    local response=$(curl -s -X POST "$API_BASE/api/wallets" \
        -H "Authorization: Bearer $API_KEY" \
        -H "Content-Type: application/json" \
        -d "{\"name\": \"$wallet_name\", \"description\": \"最终测试钱包\", \"quantum_safe\": false}")
    
    echo "创建钱包响应: $response"
    echo "$response" | grep -q "address\|id\|name"
}
