...
use defi_hot_wallet::core::config::{WalletConfig, StorageConfig, BlockchainConfig};
...
async fn test_backup_wallet_success() {
    ...
    assert!(backup_result.is_ok(), "备份钱包应该成功");
...
async fn test_backup_quantum_safe_wallet() {
    ...
    assert!(backup_result.is_ok(), "备份量子安全钱包应该成功");
...
async fn test_backup_multiple_wallets() {
    ...
    for i in 0..5 {
        let backup = manager.backup_wallet(&format!("wallet_{}", i)).await.unwrap();
        backups.push(backup);
    }
    
    assert_eq!(backups.len(), 5, "应该有5个备份");
...
async fn test_backup_and_verify_data() {
    ...
    // 备份
    let backup_data = manager.backup_wallet("backup_verify").await.unwrap();
