...
async fn test_bridge_errors_when_persistence_fails() {
    ...
    let manager = defi_hot_wallet::core::wallet_manager::WalletManager::new_with_storage(&config, failing.clone(), None)
        .await
        .expect("manager init");
    ...
    let res = manager.bridge_assets("persist_fail_wallet", "eth", "solana", "USDC", "1").await;
